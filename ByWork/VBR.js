...
function recoveryFormatData(data) {
	...
	for (var i=0; i < $rows; i++) {
		...
		$data[i][14] = '<a href=' + $data[i][14] + ' target=_blank title="Ссылка на описание">ссылка (откроется в новом окне)</a>';
		$data[i][15] = '<a href=' + $data[i][15] + ' target=_blank title="Ссылка на сайт продукта">ссылка (откроется в новом окне)</a>';
		//2.преобразует элементы в численные массивы по разделителю ","
